...
// representing the triangle's classification: 'right', 'acute',
// 'obtuse', or 'invalid'.

// You may assume that all angles have integer values, so you do not
// have to worry about floating point errors. You may also assume
// that the arguments are in degrees.
...
function isValid(angles) {
  let sumCheck = (angles.reduce((sum, val) => sum + val) === 180);
  let positiveCheck = (angles.every(angle => angle > 0));
  return sumCheck && positiveCheck;
}
